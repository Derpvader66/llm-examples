...
def load_documents(files):
    ...
    for file in files:
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix='_' + file.name) as tmp_file:
            tmp_file.write(file.getbuffer())
            temp_files.append(tmp_file.name)

        if file.type == "application/vnd.openxmlformats-officedocument.wordprocessingml.document":
            loader = UnstructuredWordDocumentLoader(temp_files[-1])
        elif file.type == "application/pdf":
            loader = UnstructuredPDFLoader(temp_files[-1])
        elif file.type == "application/vnd.ms-excel" or file.type == "text/csv":
            loader = CSVLoader(temp_files[-1])
        else:
